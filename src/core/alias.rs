...
impl AliasBase {
    pub fn new(setting_path: PathBuf) -> Result<Self, AliasError> {
    ...
    pub fn overwrite_setting(&mut self, setting: &TomlSetting) -> Result<(), AliasError> {
        ...
        files::overwrite(
            &mut setting_file,
            &self.setting_path.display().to_string(),
            &content,
        )?;
        Ok(())
    }
...
pub fn load_setting(setting_path: &PathBuf) -> Result<TomlSetting, AliasError> {
